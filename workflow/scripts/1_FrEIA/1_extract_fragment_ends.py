...
def Main():
    ...
    # Rewrite using pd.concat
    df_list = pool.map(partial(fetchData, args), ChrL)
    # Prepend OutDf to the list.
    df_list.insert(0, OutDf)

    OutDf = pd.concat(df_list, ignore_index=True)
